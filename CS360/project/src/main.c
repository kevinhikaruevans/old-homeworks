...
int init()
{
  ...
  bzero(mtable, sizeof(MTABLE) * NMTABLE);
}
...
int main(int argc, char *argv[])
{
  ...
  while (1)
  {
    ...
    printf("                 chmod|utime\n");
    printf("                 open|pfd|write|read|cat\n");
    printf("                 cp|mv\n");
    printf("                 mount|umount|switch] ");
    ...
    if (strcmp(cmd, "pfd") == 0)
      pfd();
    if (strcmp(cmd, "readfile") == 0 || strcmp(cmd, "read") == 0)
      read_file(pathname);
    if (strcmp(cmd, "cat") == 0)
      mycat(pathname);
    ...
    if (strcmp(cmd, "switch") == 0)
    {
      if (running == &proc[0])
      {
        printf("switching to proc 1\n");
        running = &proc[1];
        running->cwd = root;
      }
      else
      {
        printf("switching to proc 0\n");
        running = &proc[0];
      }
    }
