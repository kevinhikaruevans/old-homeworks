...
void Stop_Init(void) { unsigned long volatile delay;
	SYSCTL_RCGC2_R |= SYSCTL_RCGC2_GPIOE; // (a) activate clock for port E
	delay = SYSCTL_RCGC2_R;
...
void PrintSpeed(void) {
	char buffer[256];
	int dutyCycle = 100 * H / (H+L);
	sprintf(buffer, "Duty Cycle=%d%%, L=%lu\r\n", dutyCycle, L);	
